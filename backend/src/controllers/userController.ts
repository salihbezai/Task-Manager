    ...
    try {
        ...
        const usersWithTaskCounts = await Promise.all(
            users.map(async (user) => {
                const pendingTaskCount = await Task.countDocuments({ assignedTo: user._id, status: 'pending' });
                const inProgressTaskCount = await Task.countDocuments({ assignedTo: user._id, status: 'in-progress' });
